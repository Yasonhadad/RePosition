"""
Apply improved ML methodology to all 3,635 players
"""
...
def apply_ml_to_all():
    """Apply improved ML methodology to all players"""
    ...
    print(f"Applying improved XGBoost methodology to {len(players)} players...")
    ...
    for player in players:
        ...
        try:
            ...
            if processed_count % 200 == 0:
                conn.commit()
                print(f"✓ Processed {processed_count}/{len(players)} players ({processed_count/len(players)*100:.1f}%)")
    ...
    cursor.execute("""
        SELECT 
            ...
            ROUND(AVG(best_fit_score), 1) as avg_compatibility,
            best_pos,
            COUNT(*) as count_per_position
        FROM position_compatibility 
        GROUP BY best_pos
        ORDER BY count_per_position DESC
    """)
    
    print("\nPosition distribution in the new dataset:")
    for row in cursor.fetchall():
        total, avg_compat, pos, count = row
        print(f"  {pos}: {count} players (avg compatibility: {avg_compat}%)")
