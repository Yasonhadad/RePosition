"""
Update database with the new complete dataset - corrected version
"""
...
def update_players_corrected():
    ...
    # Prepare players data using correct column names
    players_data = []
    for _, row in df.iterrows():
        try:
            player_data = (
                ...
                safe_int(row.get('current_club_id', row.get('club_id'))),  # Map to club_id
                safe_int(row['OVR']),
                safe_int(row['PAC']),
                safe_int(row['SHO']),
                safe_int(row['PAS']),
                ...
                None, None, None, None, None,  # GK stats (null for field players)
                safe_str(row['League']),
                safe_str(row['Team']),
                extract_weight(row.get('Weight')),  # weight_in_kg
    ...
    # Bulk insert players using correct column order
    if players_data:
        execute_values(
            cursor,
            """
            INSERT INTO players (
                ...
                height_in_cm, current_club_name, market_value_in_eur, club_id, ovr, pac, sho, pas, dri, def, phy,
                age, weight, highest_market_value_in_eur, acceleration, sprint_speed, positioning,
                ...
                alternative_positions, play_style, gk_diving, gk_handling, gk_kicking, gk_positioning, gk_reflexes,
                league, team, weight_in_kg, image_url, league_level
            ) VALUES %s
            """,
            players_data,
            page_size=200
