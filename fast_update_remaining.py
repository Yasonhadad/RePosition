#!/usr/bin/env python3
"""
Fast update for remaining players
"""
...
def fast_update_remaining():
    """Update remaining players efficiently"""
    ...
    print(f'נותרו {remaining} שחקנים לעדכון')
    ...
    while remaining > 0:
        ...
        conn.commit()
        processed += len(batch_inserts)
        
        # Update remaining count
        cursor.execute('SELECT COUNT(*) FROM players WHERE player_id NOT IN (SELECT player_id FROM position_compatibility)')
        remaining = cursor.fetchone()[0]
        
        print(f'עובד... עודכנו {processed} שחקנים, נותרו {remaining}')
    ...
    print(f'✓ הושלם העדכון: {processed} שחקנים עודכנו בסיבוב זה')
    return processed

if __name__ == "__main__":
    fast_update_remaining()