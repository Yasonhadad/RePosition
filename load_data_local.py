    ...
    try:
        ...
        for _, row in df.iterrows():
            competitions_data.append(
                (int(row['competition_id']) if pd.notna(row['competition_id'])
                 else None, str(row['competition_name'])
                 if pd.notna(row['competition_name']) else None,
                 str(row['country_name']) if pd.notna(row['country_name']) else
                 None, str(row['competition_gender'])
                 if pd.notna(row['competition_gender']) else None,
                 str(row['competition_youth'])
                 if pd.notna(row['competition_youth']) else None,
                 str(row['competition_international'])
                 if pd.notna(row['competition_international']) else None,
                 str(row['season_name'])
                 if pd.notna(row['season_name']) else None))

        # Insert data
        execute_values(
            cur,
            """INSERT INTO competitions (id, name, country, gender, youth, international, season) 
               VALUES %s ON CONFLICT (id) DO NOTHING""", competitions_data)
        ...
        for _, row in df.iterrows():
            clubs_data.append(
                (int(row['club_id']) if pd.notna(row['club_id']) else None,
                 ...
                 if pd.notna(row['competition_id']) else None))

        # Insert data
        execute_values(
            cur, """INSERT INTO clubs (id, name, competition_id) 
               VALUES %s ON CONFLICT (id) DO NOTHING""", clubs_data)
    ...
    try:
        ...
        for _, row in df.iterrows():
            try:
                ...
                age = compute_age(row.get('date_of_birth')) or safe_int(
                    row.get('age'))
                height = safe_float(row.get('height_cm'))
                weight = extract_weight(row.get('weight')) or safe_float(
                    row.get('weight_kg'))
                ...
                # Stats
                overall_rating = safe_int(
                    row.get('overall_rating') or row.get('overall'))
                ...
                players_data.append(
                    (player_id, name, age, height, weight, foot, club_id,
                     position, overall_rating, potential, value_eur, wage_eur,
                ...
                print(
                    f"Error processing player {row.get('player_id', 'unknown')}: {e}"
                )
                continue
